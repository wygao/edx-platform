"""
Unit tests for fake XQueue implementation.
"""
...
import urllib
import time
from terrain.fakes.xqueue import FakeXQueueService


class FakeXQueueServiceTest(unittest.TestCase):

    def setUp(self):
        self.server = FakeXQueueService()
        self.url = "http://127.0.0.1:{0}".format(self.server.port)
        self.addCleanup(self.server.shutdown)

        # For testing purposes, do not delay the grading response
        self.server.set_response_delay(0)
